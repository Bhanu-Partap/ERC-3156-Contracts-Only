...
contract FlashLoanArbitrage is FlashLoanSimpleReceiverBase {
    ...
    address private exchangeContractAddress =
        0xE52b2C89049184A8C145231E1E84529771B915F7;
        ...
        FlashLoanSimpleReceiverBase(IPoolAddressesProvider(_addressProvider))
    {
        ...
        exchangeContract = IExchange(exchangeContractAddress);
    }
    ...
    function executeOperation(
        ...
        exchangeContract.depositDAI(dai.balanceOf(address(this)));
        exchangeContract.sellDAI();
    ...
    function approveUSDC(uint256 _amount) external returns (bool) {
        return usdc.approve(exchangeContractAddress, _amount);
    }

    function allowanceUSDC() external view returns (uint256) {
        return usdc.allowance(address(this), exchangeContractAddress);
    }

    function approveDAI(uint256 _amount) external returns (bool) {
        return dai.approve(exchangeContractAddress, _amount);
    }

    function allowanceDAI() external view returns (uint256) {
        return dai.allowance(address(this), exchangeContractAddress);
    }
